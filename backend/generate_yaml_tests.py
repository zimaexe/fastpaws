...
def prompt1(df, first_name, last_name):
    ...
    return str(filtred['nazov_lieku'].values)
...
def prompt2(df, first_name, last_name, drug_name):
    ...
    return filtred[filtred['nazov_lieku'].str.lower() == drug_name.lower()]['obdobie_lieku_dni'].values
...
def prompt3(df, first_name, last_name):
    ...
    if (filtred['poistovna_zaplatila_spravne'] == "Áno").all():
        ...
        not_paid = filtred[filtred['poistovna_zaplatila_spravne'] == "Nie"]['nazov_lieku'].values
        return f"No, you need to pay: {str(not_paid).replace("\n", " ")}"
...
def prompt4(df, first_name, last_name, drug_name):
    ...
    res = filtred[filtred['nazov_lieku'].str.lower() == drug_name.lower()]['cena'].values
...
def prompt5(df, first_name, last_name, drug_name):
    ...
    res = filtred[filtred['nazov_lieku'].str.lower() == drug_name.lower()]['datum_predpisu'].values
...
def prompt6(df, first_name, last_name, drug_name):
    ...
    # Перевіряємо, чи всі записи позначені як "Áno" в колонці 'liek_vybral'
    if (filtered['liek_vybral'] == "Áno").all():
        return f"Yes, you collected {drug_name}"
    else:
        return "No you did not collect all your medications, you need to took: " + str(filtered[filtered['liek_vybral'] == "Nie"]['nazov_lieku'].unique())
...
def prompt7(df, first_name, last_name, drug_name):
    ...
    return f"The quantity of {drug_name} prescribed to you is {filtered['mnozstvo'].values}"
...
def prompt9(df, first_name, last_name):
    ...
    return f"You paid {round(filtered['cena'].sum(), 2)} for all your medications"
...
def generate_10_prompts(df, name, surname, drug_name):
    ...
    print(f"""    assert:
      - type: model-graded-closedqa
        value: \"{prompt1(df, name, surname).replace("\n", ' ')}\"""")
